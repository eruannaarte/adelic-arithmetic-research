...
CONFIGS={'7': {'bank':(-10,-8,-4,0,4,8,10),'source':Q(15,10**9),'pair':Q(5,10**14),'eta':Q(11,10**8)}}
...
def check(certificate=None,kernel=None,bank='7'):
    ...
    require(certificate['schema']=='uncertain-seven-row-bank-v1','certificate schema')
    ...
    require(certificate['source_metrics']==['L2'] and
    ...
    require(len(BANK)==7 and min(BANK)>=-16 and max(BANK)<=16 and all(b-a>=2 for a,b in zip(BANK,BANK[1:])), 'fixed seven-row design class')
...
if __name__=='__main__':
    parser=argparse.ArgumentParser();parser.add_argument('--bank',choices=['7'],default='7');args=parser.parse_args()
