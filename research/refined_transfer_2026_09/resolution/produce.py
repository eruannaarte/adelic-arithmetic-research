"""Propose seven-row preconditioners, with exact whole-cell acceptance."""
...
CHOICES={'7':((-12,-10,-3,0,4,8,10),Q(74,10**10),Q(2,10**14))}
...
def build(bank):
    ...
    return {'schema':'refined-seven-row-bank-v1','kernel_sha256':hashlib.sha256(path.read_bytes()).hexdigest(),
            ...
            'source_metrics':['L2'],'H1_metric_upper':'41',
            'cells':cells,'case_count':len(cases),'producer_attempts':attempts}


if __name__=='__main__':
    p=argparse.ArgumentParser();p.add_argument('--bank',choices=['7'],required=True);p.add_argument('--write',action='store_true');args=p.parse_args()
    d=build(args.bank)
