...
def model_error_check():
    ...
    rho=Q(1,10**9)
    require(A1*A1*(2*len(BANK))*entry*entry<rho*rho,'physical operator remainder')
    # Elementary pi<22/7 and sin(x)<=x give both H1 metrics <=41 I.
    require(1+4*Q(22,7)**2<41,'source metric bound')
    return rho


def check(certificate=None,kernel=None):
    path=HERE/'kernel.json'
    kernel=json.loads(path.read_text()) if kernel is None else kernel
    certificate=json.loads((HERE/'certificate.json').read_text()) if certificate is None else certificate
    require(certificate['schema']=='separated-bank-preconditioned-v1','certificate schema')
    ...
    require(certificate['unnormalized_source_floor']=='7/100000000' and
            certificate['unnormalized_pair_floor']=='1/100000000000','source or pair floor')
    ...
    for cell in certificate['cells']:
        ...
        for record in cell['records']:
            ...
            floor=Q(7,10**8) if dimension==2 else Q(1,10**11)
    ...
    rho=model_error_check()
    mu=A0*A0*Q(7,10**8);pair=A0*A0*Q(1,10**11)
    metrics={}
    for name in certificate['source_metrics']:
        factor=1 if name=='L2' else 41
        ...
        require(delta*delta<mu/factor*Q(1,10**6),'source error target')
        metrics[name]={'sensor_relative_radius':str(eta),'operator_relative_radius':str(rho),
    ...
    return {'verified':True,'direct_rows':[v+500 for v in BANK],'channel_count':11,'target_indices':[490,510],
