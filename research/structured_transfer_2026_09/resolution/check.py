...
BANK=(490,492,496,500,504,508,510)
H=Q(1,2000000);DG=Q(1,400000);ETA=Q(11,10**8);RHO=XI=Q(1,10**9)
MU=Q(1083,51200000000);LAM=Q(361,5120000000000000)
LIMITS={'joint_forward':Q(66,10**9),'joint_inverse':Q(28,10**6),'time_forward':Q(24,1000),'potential_forward':Q(24,1000),'time_inverse':Q(18),'potential_inverse':Q(8)}
...
def remainders():
    ...
    dg_squared=Q(16,9)*14*eg**2
    dt_squared=14*(e0/6+Q(4,3)*et)**2
...
def consequence(bounds=None,h=H,dg=DG):
    ...
    require(2*total*total<LAM,'label tube gate')
    ...
    require(total*total>MU*Q(1,1000)**2,'comparison must expose failed global inverse gate')
    require(Q(4,5)-DG>=0,'entire physical potential family PSD')
    return {'verified':True,'sensor_relative_radius':str(ETA),'clock_radius':str(H),'potential_radius':str(DG),'potential_tolerance_factor_vs_prior_generic':10000,'source_relative_accuracy_target':'1/1000','source_relative_error_upper':str(accuracy),'label_total_relative_radius':str(total),'label_pair_floor':str(LAM),'source_floor':str(MU),'structured_direction_radius':str(b['joint_forward']),'structured_inverse_radius':str(b['joint_inverse']),'unstructured_remainder_radius':str(free),'old_global_inverse_gate_fails_at_same_rectangle':True,'label_separation_verified':True,'source_accuracy_verified':True,'remainders':rem}
...
def verify(document=None,bits=320):
    ...
    for path in (PRIOR/'kernel.json',HERE/'kernel_derivative.json'):
    ...
    with ctx.workprec(bits):
        p=kernel(PRIOR/'kernel.json');q=kernel(HERE/'kernel_derivative.json',True)
