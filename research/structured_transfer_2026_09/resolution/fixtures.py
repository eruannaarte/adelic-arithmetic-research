...
def complete_physical_radius():
    ...
    assert Q(16,9)*7*(image+boundary)**2<Q(1,10**44)
    return Q(1,10**22)

def build(bits=256):
    started=time.perf_counter();cases=[];radius=complete_physical_radius()
    pairs=[(Q(1),Q(1)+check.H,1),(Q(3,2),Q(3,2)-check.H,-1),(Q(3,2),Q(3,2)+check.H,1),(Q(2),Q(2)-check.H,-1)]
