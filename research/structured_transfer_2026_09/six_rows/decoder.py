...
Data are the selected real coordinates z=y/(1+tau)^(1/4), in the certified bank order.
...
class CertifiedBank:
    ...
    def __init__(self,bank="6"):
        kernel=json.loads((consumer.PRIOR/'kernel.json').read_text())
        self.summary=consumer.check(kernel=kernel,bank=bank)
        self.bank=tuple(v-500 for v in self.summary["direct_rows"])
        self.profiles=dict(self.summary["metrics"])
    ...
    def matrices(self,tau):
        ...
        return {500+j:tuple(tuple(value(self._coeff[abs(sensor-j)][port],tau)
                                      for port in (0,1)) for sensor in self.bank)
    ...
    def decode(self,data,tau,metric='L2',numerical_relative_radius='0'):
        ...
        if type(data) not in (list,tuple) or len(data)!=len(self.bank):raise ValueError('wrong number of real bank coordinates')
        ...
        factor=1
        eta=Q(self.profiles[metric]['sensor_relative_radius'])
