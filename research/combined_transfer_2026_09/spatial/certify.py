...
def check(certificate=None,kernel=None,bank='6'):
    ...
    metadata={'schema':'polynomial-weighted-pair-combined-six-row-v1','kernel_sha256':hashlib.sha256(path.read_bytes()).hexdigest(),
              ...
              'normalization_lower':'19/16','normalization_upper':'4/3','source_relative_accuracy_target':'1/1000','metric':'L2','label_tube_radius':'13/400000000','clock_radius':'1/100000000','potential_radius':'1/100000000'}
    for key,value in metadata.items():require(same_json(doc[key],value),'changed certificate contract: '+key)
    coeff=interval_coefficients(kernel);rho=model_error_check(6);eta=Q(3,10**8);xi=Q(1,10**9);delta=Q(13,400000000)
    ...
    metric={'source_relative_accuracy_target':str(accuracy),'sensor_relative_radius':str(eta),'operator_relative_radius':str(rho),
            ...
            'individual_approximate_map_floor':str(mu),'unstructured_source_relative_error_squared_upper':str(delta*delta/mu),
            'note':'This file proves the enlarged nominal tubes; check.py supplies actual-model clock/potential transfer.',
    ...
    return {'verified':True,'direct_rows':[485,493,499,501,507,515],'channel_count':6,'target_indices':[490,510],
            'nominal_time_interval':['1','2'],'individual_cases':21,'pair_cases':210,'whole_cell_records':count,
