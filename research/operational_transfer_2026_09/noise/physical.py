...
All phases, weights and polynomial columns are reconstructed, not loaded from a
...
def load_prior():
    spec=importlib.util.spec_from_file_location('_operational_tail_consumer',PRIOR/'check_certificate.py')
    module=importlib.util.module_from_spec(spec);spec.loader.exec_module(module)
    document=json.loads((PRIOR/'evidence.json').read_text())
    module.check(document,extended=True)
    return document
...
class PhysicalModel:
    def __init__(self,design='multi',degree=8,bits=256):
        if design not in ('multi','outer') or type(degree) is not int or degree not in (6,8,10,12):raise ValueError('unsupported certified design/degree')
