...
def verify(doc):
    check.require(doc['schema']=='seven-row-specific-obstruction-v1','obstruction schema')
    ...
    rho=check.model_error_check(len(ROWS));eta=Q(3,10**8)
...
def produce():
    ...
    return {'schema':'seven-row-specific-obstruction-v1','bank_offsets':list(ROWS),
            'time':str(TIME),'target_offsets':list(LABELS),'source_pair':v,
            'sensor_relative_radius':'3/100000000',
            'kernel_sha256':hashlib.sha256((check.PRIOR/'kernel.json').read_bytes()).hexdigest()}


if __name__=='__main__':
    p=argparse.ArgumentParser();p.add_argument('--write',action='store_true');args=p.parse_args()
    doc=produce() if args.write else json.loads((HERE/'obstruction_7.json').read_text())
    result=verify(doc)
    if args.write:
        (HERE/'obstruction_7.json').write_text(json.dumps(doc,indent=2)+'\n')
        (HERE/'obstruction_7_checked.json').write_text(json.dumps(result,indent=2)+'\n')
